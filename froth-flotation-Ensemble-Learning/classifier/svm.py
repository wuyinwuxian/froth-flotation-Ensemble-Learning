...
'''训练'''
classifier=svm.SVC(kernel='linear',gamma=0.2,max_iter=1e5,decision_function_shape ='ovo',C=7,probability=True)    #定义机器学习算法

num_classes = 8                                # 定义8分类
name = 'svm'                                   # 采用的方法名
trainx = np.loadtxt("../dataset/trainx.txt")   # 导入训练数据
trainy = np.loadtxt("../dataset/trainy.txt")
x_train = np.array([[x[2], x[6], x[9], x[13], x[17]] for x in trainx])  # 特征选择以后的特征
x_vals_train = normalize(x_train, axis=0, norm='max')                   # 归一化
...
np.save('../evaluation_index_mat/' + name + '_index.npy', index)                                  # 注意带上后缀名


'''测试'''
testx = np.loadtxt("../dataset/testx.txt")                               # 导入测试数据
testy = np.loadtxt("../dataset/testy.txt")
x_test = np.array([[x[2], x[6], x[9], x[13], x[17]] for x in testx])     # 特征选择以后的特征
x_vals_test = normalize(x_test, axis=0, norm='max')                      # 归一化
y_vals_test = np.array(testy).astype(int) - 1                            # 类别从 0 开始，所以减1

predict = classifier.predict(x_vals_test)                                # 预测结果
predict_pro = classifier.predict_proba(x_vals_test)                      # 预测结果的概率
np.save('../probabilistic_output/' + name + '_output.npy', predict_pro)  # 注意带上后缀名
acc = classifier.score(x_vals_test, y_vals_test)                         # 准确率


'''画图'''
def plot_matrix():

    json_label_path = '../class_indices.json'            # 读取标签评语
